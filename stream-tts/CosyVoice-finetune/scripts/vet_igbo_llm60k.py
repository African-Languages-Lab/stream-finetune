...
# epoch_6_whole (59,556 steps, cv 3.5359, +4.3%) is the DEEPEST igbo llm before the cliff:
# the very next checkpoint, 59,999, jumps to 3.7257 (+9.9%) and it climbs to 7.12 by 204k.
# Paired against epoch_2_whole (25,553, cv 3.3892), the lowest-CV pick, on the same flow.
BUILDS = {
    "individual_igbo-llm60k": (f"{CK}/individual/individual_ig-NG",
                               "epoch_6_whole.pt", "epoch_31_whole.pt"),
}
...
def main():
    ...
    json.dump(manifest, open(os.path.join(OUT, "manifest_igbo_llm60k.json"), "w"), indent=1)
